...
//				  if(propDao.get(accom.getIdp()).getIdu() == user.getId()) {
				  ...
				  res = Response //return 201 and Location:
					...
					throw new CustomBadRequestException("No puedes realizar un actualización de un alojamiento que no es tuyo");
				}

//			  } else {
//				throw new CustomNotFoundException("No se puede actualizar una habitacion que no existe");
//			}
			  
//		  } else {
//			throw new CustomBadRequestException("Debes iniciar sesión para poder actualizar una habitacion a un alojamiento");
//		}
		  ...
		  Connection conn = (Connection) sc.getAttribute("dbConn");
			...
			if(user != null) {
				if(propDao.get(accomDao.get(ida).getIdp()).getIdu() == user.getId()) {
					...
					throw new CustomBadRequestException("No puedes actualizar una habitación a un alojamiento que no este tuyo.");
				}
		  } else {
				throw new CustomBadRequestException("Debes iniciar sesión para poder añadir una habitacion a un alojamiento");
			}

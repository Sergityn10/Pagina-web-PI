...
public class ReviewResource {
	...
	@Context
		  ...
		  Connection conn = (Connection) sc.getAttribute("dbConn");
			...
			if(propDao.get(review.getIdp()) != null && user.getId()==review.getIdu() && reviewDao.get(review.getIdp(), review.getIdu()) == null) {
				...
				res = Response //return 201 and Location: /orders/newid 
				  .created(
		  uriInfo.getAbsolutePathBuilder() .path(Long.toString(review.getIdp())) .build())
		  .contentLocation( uriInfo.getAbsolutePathBuilder() .path(Long.toString(review.getIdp()))
		  .build()).build();
				  return res; 
			}
			else {
	  ...
	  public Response postReview(@PathParam("reviewid")long idp,
			  @FormParam("num_valoracion") int valoracion, 
	  ...
	  public Response putReview(@PathParam("idu") long idu,Review review, 
		  ...
		  if(reviewDao.get(review.getIdp(), review.getIdu()) != null) {
			  ...
			  logger.info("PUT utilizando un JSON de la review: idp->" + review.getIdp()+ ", idu->" + review.getIdu());
			  res = Response //return 201 and Location: 
				   .created(
					uriInfo.getAbsolutePathBuilder()
						   .path(Long.toString(review.getIdp()))
						   .build())
				   .contentLocation(
					uriInfo.getAbsolutePathBuilder()
					       .path(Long.toString(review.getIdp()))
			  ...
			  else throw new CustomBadRequestException("No puedes realizar un actualización de una review que no es tuya");
		  
		  }
		  else throw new CustomNotFoundException("No se ha podido encontrar o actualizar la review deseada");		  
	  ...
	  @Path("/{reviewid: [0-9]+}")
	  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	  public Response updateReview(@PathParam("reviewid")long idp,
			  @FormParam("num_valoracion") int valoracion, 
			  @FormParam("descripcion") String descripcion,
			  @Context HttpServletRequest request) {
		  Connection conn = (Connection) sc.getAttribute("dbConn");
	  ...
	  @DELETE	  
	  public Response deleteAllOrdersUser(
			  					  @Context HttpServletRequest request) {
